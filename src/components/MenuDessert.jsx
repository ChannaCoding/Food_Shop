...
const MenuDessert = () => {
  ...
  const [filteredDesserts, setFilteredDesserts] = useState(khmerDessert);
  ...
  useEffect(() => {
    const filtered = khmerDessert.filter(dessert =>
  ...
  const currentItems = filteredDesserts.slice(indexOfFirstItem, indexOfLastItem);
      ...
      <div className='max-w-7xl mx-auto px-6 md:px-14'>
        ...
        <div data-aos="fade-down" className="mb-16 border-l-4 border-[#2D4A22] pl-6 relative">
          <h1 className="text-3xl md:text-4xl font-bold text-[#2D4A22] uppercase tracking-tight">
             KHMER-FRESH <span className="text-[#F58220] font-light">/ {t('menu.desserts') || 'Desserts'}</span>
          </h1>
          <p className="text-gray-400 font-medium mt-2 text-sm tracking-widest uppercase">
            {t('menu.dessertsSubtitle') || 'Sweet & Delightful Traditional Treats'}
          </p>
        ...
        <div className='max-w-xl mb-12' data-aos="fade-up">
          ...
          <p className='text-[10px] font-bold text-gray-400 mt-3 uppercase tracking-widest'>
            Total: {filteredDesserts.length} Items Found
          </p>
        </div>
          ...
          {currentItems.length > 0 ? (
            currentItems.map((s, index) => (
              ...
              >
                ...
                {/* Content Section (អក្សរតូចល្មម ស្អាត) */}
            ...
            <div className='col-span-full text-center py-20 border-2 border-dashed border-gray-100 rounded-xl'>
              <h3 className='text-xl font-bold text-gray-400 uppercase tracking-widest'>No desserts found</h3>
